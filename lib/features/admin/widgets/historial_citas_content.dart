...
class HistorialCitasFunciones {
  ...
  static Future<List<HistorialCitaVista>> cargarHistorial({
    ...
    final historial = citas.map((cita) {
      final fechaBase = cita.fechaHora ?? cita.fechaOrden;
      final dueno =
          usuariosPorId[cita.idUsuario]?.nombreCompleto.trim() ?? 'Sin dueno';
      final doctor =
          medicosPorId[cita.idMedico]?.nombreCompleto.trim() ?? 'Sin doctor';
  ...
  static List<HistorialCitaVista> filtrarHistorial({
    ...
    final filtradas = historial.where((cita) {
      final coincideEstado =
          estadoSeleccionado == estadoTodos || cita.estado == estadoSeleccionado;
      final coincideEspecie =
          especieSeleccionada == especieTodas || cita.especie == especieSeleccionada;
