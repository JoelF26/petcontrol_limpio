...
// Centraliza listas visibles desde assets y usa fallbacks si el archivo cambia o falla.
class CatalogosJsonService {
  CatalogosJsonService({required CatalogosRepository catalogosRepository})
  ...
  Future<List<String>> obtenerEstadosCreacionCitaAdmin() async {
    final data = await _leerCatalogos();
    final estados = _leerLista(data, const <String>[
      'citas',
      'estados_creacion_admin',
    ], _fallbackEstadosCreacionCitaAdmin);
    return _normalizarEstadosCreacionAdmin(estados);
  }
  ...
  Future<Map<String, dynamic>> _leerCatalogos() {
    return _catalogosRepository.leerCatalogos();
  }
  ...
  List<String> _leerLista(
    ...
    // Navega una ruta anidada como ['citas', 'estados'] y valida cada nivel.
    dynamic actual = data;
  ...
  List<String> _normalizarEstadosCreacionAdmin(List<String> estados) {
    final normalizados = <String>[];
    for (final estado in <String>[...estados, 'proxima', 'confirmada']) {
      final limpio = estado.trim().toLowerCase();
      if (limpio.isEmpty || normalizados.contains(limpio)) {
        continue;
      }
      if (limpio == 'proxima' || limpio == 'confirmada') {
