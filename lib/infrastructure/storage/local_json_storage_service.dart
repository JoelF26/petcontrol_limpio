...
class LocalJsonStorageService {
  ...
  Future<void> inicializar() {
    ...
    // Reutiliza la misma inicialización si varias pantallas piden datos al abrir.
  ...
  // Lee datos de ejemplo desde assets y los normaliza antes de persistirlos.
  Future<Map<String, dynamic>> _crearBaseInicialDesdeSeed() async {
    try {
      ...
      // Si el asset falla, se deja una estructura válida para que la app siga operando.
    }
    return _crearBaseInicialVacia();
  }

  // Sección: normalización de base
  // Repara bases incompletas o antiguas agregando entidades faltantes como listas.
  ...
  Future<void> _ejecutarInicializacion() async {
    try {
      ...
      // Acepta tanto mapas tipados como mapas dinámicos para tolerar cambios del decoder.
  ...
  Future<Map<String, dynamic>> _leerBase() async {
    ...
    // Si se detectan entidades dañadas o ausentes, se corrige y se guarda la reparación.
  ...
  // Convierte listas dinámicas del JSON en registros map tipados para los modelos.
